...
impl Assignment {
    ...
    fn any_overlap(&self, other: &Self) -> bool {
        let self_overlaps_other = (other.start >= self.start && other.start <= self.end) ||
            (other.end <= self.end && other.end >= self.start);
        let other_overlaps_self = (self.start >= other.start && self.start <= other.end) ||
            (self.end <= other.end && self.end >= other.start);
...
fn main() {
    ...
    let full_contained_assignments_count = parse_assignments(filename)
        ...
        .filter(|ap| ap.0.any_overlap(&ap.1))
        .count();
